...
public extension ResponseTransformer
    ...
    public func logTransformation(_ result: Response) -> Response
        {
        log(.pipeline, ["  ├╴Applied transformer:", self, "\n  │ ↳", result.summary()])
        return result
        }
    }
    ...
    func process(_ response: Response) -> Response
        ...
        if let contentType = contentType,
           contentTypeMatcher.matches(contentType)
            {
            log(.pipeline, ["  ├╴Transformer", self, "matches content type", debugStr(contentType)])
        ...
        guard let typedEntity = entity.withContentRetyped() as Entity<InputContentType>? else
            ...
            switch mismatchAction
                ...
                case .skip,
                    ...
                    log(.pipeline, [self, "skipping transformer because its mismatch rule is", mismatchAction, ", and it expected content of type", InputContentType.self, "but got a", type(of: entity.content)])
    ...
    private func processError(_ error: RequestError) -> Response
        ...
        if transformErrors, let errorData = error.entity
            ...
            switch processEntity(errorData)
                ...
                case .failure(let error):
                    log(.pipeline, ["Unable to parse error response body; will leave error body unprocessed:", error])
                }
            }
